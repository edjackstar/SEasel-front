<template>
  <div>
    <div class="title default-top-margin">{{ $t('edit_concert_title') }}</div>
    <el-form
      class="default-top-margin default-form-width default-left-margin"
      :model="concert"
      size="mini"
      label-width="140px">
      <el-form-item prop="music_school" :label="$t('edit_concert_label_music_school')">
      ...
      <el-form-item prop="date" :label="$t('edit_concert_label_date')">
...
export default class StudentPage extends Vue{
  ...
  async mounted(){
    this.concert = await this.$api.admin.concert.getConcert(+this.$route.params.id)
    try{
  ...
  async submit() {
    ...
    try {
      this.loading = true
      await this.$api.admin.concert.updateConcert(concert)
      this.$router.push(this.localePath('/admin/concerts'))
    } catch (e) {
      this.$message({
        message: this.$t('edit_concert_error').toString(),
        type: 'error'
      })
    } finally {
      this.loading = false
