<template>
  <div>
    <div class="title default-top-margin">{{ $t('edit_semester_title') }}</div>
    <el-form
      class="default-top-margin default-form-width default-left-margin"
      :model="semester"
      size="mini"
      label-width="140px">
      <el-form-item prop="music_school" :label="$t('edit_semester_label_music_school')">
      ...
      <el-form-item prop="number" :label="$t('edit_semester_label_number')">
        <el-input v-model="semester.number" />
      </el-form-item>
      <el-form-item prop="count" :label="$t('edit_semester_label_composition_count')">
        <el-input v-model="semester.composition_count" />
      </el-form-item>
      <el-form-item prop="min_difficulty" :label="$t('edit_semester_label_min_difficulty')">
        <el-input v-model="semester.min_difficulty" />
      </el-form-item>
      <el-form-item prop="max_difficulty" :label="$t('edit_semester_label_max_difficulty')">
...
export default class StudentPage extends Vue{
  ...
  async mounted(){
    this.semester = await this.$api.admin.semester.getSemester(+this.$route.params.id)
    try{
